...
const BlogContent = ({ posts }: Props) => {
  return (
    <Container className="bg-gray-100 py-20 px-10 flex flex-col gap-10">
      {posts.map((post) => (
        <div key={post?._id} className="blog-container">
          <Link
            href={{
          ...
          >
            <div className="flex flex-col md:flex-row gap-10 bg-white rounded-md hover:shadow-md duration-200">
              <div className="w-full md:w-3/5 group overflow-hidden rounded-tl-md rounded-bl-md relative">
                <Image
                  // Ensure the asset is checked before calling urlFor()
                  src={
                    post?.mainImage?.asset?._ref
                      ? urlFor(post.mainImage).url()  // Use url() after verifying it's an ImageUrlBuilder object
                      : "/fallback-image.jpg"
                  }
                  width={500}
                  height={500}
                    ...
                    <Image
                      ...
                      src={
                        post?.author?.image?.asset?._ref
                          ? urlFor(post.author.image).url()  // Use url() after verifying it's an ImageUrlBuilder object
                          : "/fallback-author.jpg"
                      }
                      width={200}
                      height={200}
          ...
          {/* Add the CommentSection below the blog content */}
          <CommentSection blogId={post?._id as string} />
        </div>
      ))}
    </Container>
  );
